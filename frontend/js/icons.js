/* Notepay Icons Library - v3 (Professional Overhaul) */
(function() {
    const icons = {
        // Essentials
        ...
        "more-vertical": '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="1"/><circle cx="12" cy="5" r="1"/><circle cx="12" cy="19" r="1"></svg>',
        ...
        "users": '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path><circle cx="9" cy="7" r="4"></circle><path d="M23 21v-2a4 4 0 0 0-3-3.87"></path><circle cx="17" cy="7" r="4"></circle></svg>',
        "crown": '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m2 4 3 12h14l3-12-6 7-4-7-4 7-6-7zm3 16h14"></path></svg>',
        "badge-check": '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"></path><path d="m9 12 2 2 4-4"></path></svg>',
        "calendar": '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg>',
        "globe": '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="2" y1="12" x2="22" y2="12"></line><path d="M12 2a15.3 15.3 0 0 1 0 20 15.3 15.3 0 0 1 0-20"></path></svg>',
        ...
        "key": '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 2l-2 2m-7.61 7.61a5.5 5.5 0 1 1-7.778 7.778 5.5 5.5 0 0 1 7.777-7.777zm0 0L15.5 7.5m0 0l3 3m-3-3l-4-4"></path></svg>',
        ...
        "pin": '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 10V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v2a10 10 0 0 0 5 8.66V22h8v-3.34A10 10 0 0 0 21 10z"></path></svg>',
        ...
        "scanner": '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 7V5a2 2 0 0 1 2-2h2"></path><path d="M17 3h2a2 2 0 0 1 2 2v2"></path><path d="M7 21H5a2 2 0 0 1-2-2v-2"></path><path d="M21 17v2a2 2 0 0 1-2 2h-2"></path><line x1="5" y1="12" x2="19" y2="12"></line></svg>'
    };
    ...
    window.npIcon = function(name, options = {}) {
        const icon = icons[name] || icons["alert-circle"];
        const size = options.size || 24;
        const tone = options.tone || "currentColor";
        const color = toneMap[tone] || tone;
        
        let styledIcon = icon.replace('<svg ', `<svg width="${size}" height="${size}" style="color:${color}" `);
        return styledIcon;
    };

    window.iconLabel = function(icon, text, options = {}) {
        return `<span class="np-icon-label">${window.npIcon(icon, options)} ${text}</span>`;
    };

    window.initIcons = function() {
        document.querySelectorAll('[data-np-icon]').forEach(el => {
            const name = el.getAttribute('data-np-icon');
            const size = parseInt(el.getAttribute('data-np-size')) || 24;
            const tone = el.getAttribute('data-np-tone') || "currentColor";
            el.innerHTML = window.npIcon(name, { size, tone });
        });
    };

    window.hydrateNpIcons = window.initIcons;
